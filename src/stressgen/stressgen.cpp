...
class TGenerator : public Generator_interface
{
public:
    virtual std::string generate() const override 
    { 
        return std::to_string(std::rand()%80 + 1); 
    }
};

class KGenerator : public Generator_interface
{
public:
    virtual std::string generate() const override 
    { 
        return std::to_string(1+(std::rand()%99 + 1)/100.0); 
    }
};
...
std::string generateLine(std::string str, int line_no, const std::string & control_character)
{
    std::vector<std::pair<std::string,std::shared_ptr<Generator_interface>>> replaces
    {
        ...
        {control_character+"T", std::make_shared<TGenerator>()},
        {control_character+"K", std::make_shared<KGenerator>()},
    };
